...
export default function BoardList( props ){
    ...
    return(<>
        ...
        {/* 삼항 연산자를 이용한 조건부 랜더링 */}
        {
            ...
            (<> </>)
            :
            (<> <button type="button" onClick={DeleteSearch } > 검색제거 </button> </>)
        }
          ...
          <Table sx={{ minWidth: 650 }} aria-label="simple table">
            ...
            <TableHead>
              <TableRow>
                ...
                <TableCell style={{width : '50%'}} align="right">제목</TableCell>
                <TableCell style={{width : '10%'}} align="right">작성자</TableCell>
                <TableCell style={{width : '10%'}} align="right">작성일</TableCell>
                <TableCell style={{width : '10%'}} align="right">조회수</TableCell>
              </TableRow>
            </TableHead>
             {/*테이블 내용 구역*/}
            <TableBody>
              { pageDto.boardDtos.map((row) => // map is not a function
                <TableRow key={row.name}  sx={{ '&:last-child td, &:last-child th': { border: 0 } }} >
                  ...
                  <TableCell align="right">{formatDateTime(row.cdate)}</TableCell>
